...
def main(start):
    ...
    while run:
        ...
        if (keys[pygame.K_UP] or keys[pygame.K_w])and mainCharacter.y - playerVel > 0:
            mainCharacter.y -= playerVel
        if (keys[pygame.K_DOWN] or keys[pygame.K_s]) and mainCharacter.y + playerVel + 50 < HEIGHT:
            mainCharacter.y += playerVel
        if (keys[pygame.K_RIGHT] or keys[pygame.K_d]) and mainCharacter.x + playerVel + 50 < WIDTH:
            mainCharacter.x += playerVel
        if (keys[pygame.K_LEFT] or keys[pygame.K_a]) and mainCharacter.x - playerVel > 0:
            mainCharacter.x -= playerVel
...
def main_menu():
    ...
    while run:
        ...
        if keys[pygame.K_h]:
            ctypes.windll.user32.MessageBoxW(0, "To win, simply reach the chequered flag. Use the arrow keys or awsd to move Sid. Avoid snakes, cannons and the cannonballs fired. Your total time will be recorded. There are 5 rounds.", "Instructions", 1)
